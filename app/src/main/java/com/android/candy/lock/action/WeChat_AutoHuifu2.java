...
public class WeChat_AutoHuifu2 extends BaseAction {
    ...
    public void event(AccessibilityEvent event) {
        ...
        List<AccessibilityNodeInfo> titles2 = nodeInfo.findAccessibilityNodeInfosByViewId("com.tencent.mm:id/gz");//会话title
        ...
        if (index==0&&event.getEventType() != AccessibilityEvent.TYPE_WINDOW_CONTENT_CHANGED) {
            ...
            if (listview.isEmpty() == false) {
                AccessibilityNodeInfo lastItem = listview.get(0).getChild(listview.get(0).getChildCount() - 1);//获取listview最后一个item
                //获取item中头像view
                AccessibilityNodeInfo itemHeadInfos = lastItem.findAccessibilityNodeInfosByViewId("com.tencent.mm:id/io").get(0);
                //获取当前会话用户的name(顶部)
                ...
                if (itemHeadInfos.getContentDescription().toString().contains(titleUserName.getText().toString())) {
                    List<AccessibilityNodeInfo> items = lastItem.findAccessibilityNodeInfosByViewId("com.tencent.mm:id/iq");//获得消息TextView
                ...
                edits = nodeInfo.findAccessibilityNodeInfosByViewId("com.tencent.mm:id/a6g");//获得编辑框
            }
            } catch (Exception e) {
        ...
        if (index == 2) {
            try {
                List<AccessibilityNodeInfo> edits = nodeInfo.findAccessibilityNodeInfosByViewId("com.tencent.mm:id/a6g");//获得编辑框
                List<AccessibilityNodeInfo> titles = nodeInfo.findAccessibilityNodeInfosByViewId("com.tencent.mm:id/gz");//获得回话标题，含有“()”的，则为群聊
