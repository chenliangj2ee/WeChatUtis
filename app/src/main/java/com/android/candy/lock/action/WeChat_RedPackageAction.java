...
//            return;
    ...
    private void openPacket() {
        try {
            ...
            if (nodeInfo != null) {
                List<AccessibilityNodeInfo> list = nodeInfo.findAccessibilityNodeInfosByViewId("com.tencent.mm:id/bp6");//获得【开】按钮
        ...
        try {
            ...
            if (nodeInfo != null) {
                ...
                if (listview.isEmpty() == false) {
                    AccessibilityNodeInfo info = listview.get(0).getChild(listview.get(0).getChildCount() - 1);//获取listview最后一个item
                    if(info!=null){
                        ...
                        if (last.isEmpty()==false) {
                            ...
                            //获取item中头像view
                            AccessibilityNodeInfo itemHeadInfos= info.findAccessibilityNodeInfosByViewId("com.tencent.mm:id/io").get(0);

                            //获取当前会话用户的name(顶部)
                            AccessibilityNodeInfo titleUserName= nodeInfo.findAccessibilityNodeInfosByViewId("com.tencent.mm:id/gz").get(0);
                            ...
                            if(itemHeadInfos.getContentDescription().toString().contains(titleUserName.getText().toString())){
                                ...
                                //获取红包views
                                List<AccessibilityNodeInfo> list = nodeInfo.findAccessibilityNodeInfosByViewId("com.tencent.mm:id/a8q");
