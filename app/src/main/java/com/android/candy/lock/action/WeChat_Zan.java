...
public class WeChat_Zan extends BaseAction {
    ...
    public void event(AccessibilityEvent event) {
        ...
        List<AccessibilityNodeInfo> listviews = nodeInfo.findAccessibilityNodeInfosByViewId("com.tencent.mm:id/cvm");//获得朋友圈ListView
        ...
        if (index == 0) {
            try {
                log("当前步骤：" + index);
                List<AccessibilityNodeInfo> pingluns = nodeInfo.findAccessibilityNodeInfosByViewId("com.tencent.mm:id/cso");//获得评论按钮
